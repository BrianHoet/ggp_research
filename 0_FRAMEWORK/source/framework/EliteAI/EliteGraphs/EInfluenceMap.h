	...
	void InfluenceMap<T_GraphType>::PropagateInfluence(float deltaTime)
	{
		...
		if(m_TimeSinceLastPropagation >= m_PropagationInterval)
		{
			for(auto& node : m_Nodes)
			{
				...
				{
					const float newInfluence{ node->GetInfluence() * expf(-connection->GetCost() * GetDecay()) };
					if (newInfluence > highestInfluence)
					{
						highestInfluence = newInfluence;
					}
				}
				highestInfluence = Lerp(node->GetInfluence(), highestInfluence, GetMomentum());
				m_InfluenceDoubleBuffer.push_back(highestInfluence);
			}
