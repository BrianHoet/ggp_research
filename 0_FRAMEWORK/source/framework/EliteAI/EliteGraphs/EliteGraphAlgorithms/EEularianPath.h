...
namespace Elite
{
	...
	inline Eulerianity EulerianPath<T_NodeType, T_ConnectionType>::IsEulerian() const
	{
		...
		for(auto n : nodes)
		{
			...
			//&1 to check if odd number => in binary, every odd number ends with 1
	...
	inline std::vector<T_NodeType*> EulerianPath<T_NodeType, T_ConnectionType>::FindPath(Eulerianity& eulerianity) const
	{
		...
		if (eulerianity == Eulerianity::semiEulerian)
			...
			{
				auto connections = graphCopy->GetNodeConnections(i);
				//&1 to check if odd number => in binary, every odd number ends with 1
				if (connections.size() & 1)
				{
					currentNode = i;
		...
		if(graphCopy->GetNodeConnections(currentNode).size() > 0)
		{
			...
			currentNode = graphCopy->GetNodeConnections(currentNode).begin();
			graphCopy->RemoveConnection(currentNode, nodeStack[currentNode-1]);
		}
		else if(graphCopy->GetNodeConnections(currentNode).size() == 0)
		{
